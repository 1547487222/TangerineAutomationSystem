...
using static System.Foundation.Modules.Arms.MechanicalArmWithTransTool;

namespace System.Foundation.Modules.Arms
{
    [DisplayName("搬回模块-过渡点")]
    public class MechanicalArmMoveBackWithTransTool : SyncInputModuleToolBase
    {
        private Judger _Judger;
        public MechanicalArmMoveBackWithTransData mechanicalArmMoveBackWithTransData = new MechanicalArmMoveBackWithTransData();
        public class MechanicalArmMoveBackWithTransData: ModuleData
        {
            [DisplayName("坐标起始地址")]
        ...
        public override string DefineName => "搬回模块-过渡点";
        ...
        {
            InsetPin("输入触发搬回信号",this,typeof(QData), PinType.Input);
        ...
        {
            DataContext = mechanicalArmMoveBackWithTransData;
        ...
        public override bool InitEnd()
        {
            ...
            {
                var state = await h5UTcp?.ReadSingleValueAsync<short>(mechanicalArmMoveBackWithTransData.StateAddress);
                return state == mechanicalArmMoveBackWithTransData.StateCode;
            })
        ...
        public override async Task<bool> ExexuteAsync(Dictionary<PinInfo, QData> pinDatas, ToolExecutionContext toolContext)
        {
            ...
            mechanicalArmMoveBackWithTransData = Context<MechanicalArmMoveBackWithTransData>();
            posStart.X += mechanicalArmMoveBackWithTransData.X_Start_Offset;
            posStart.Y += mechanicalArmMoveBackWithTransData.Y_Start_Offset;
            posStart.Z += mechanicalArmMoveBackWithTransData.Z_Start_Offset;
            posEnd.X += mechanicalArmMoveBackWithTransData.X_End_Offset;
            posEnd.Y += mechanicalArmMoveBackWithTransData.Y_End_Offset;
            posEnd.Z += mechanicalArmMoveBackWithTransData.Z_End_Offset;
            //偏移后位置记录
            ...
            if (mechanicalArmMoveBackWithTransData.ContainAngle)
            {
                data =
               [
                ...
                mechanicalArmMoveBackWithTransData.ClawOpenPos,
                mechanicalArmMoveBackWithTransData.ClawOpenAngle,
                mechanicalArmMoveBackWithTransData.ClawClosePos,
                mechanicalArmMoveBackWithTransData.ClawCloseAngle
               ];
            }
            else
            {
            ...
            await GetModular().WriteParameterAsync(mechanicalArmMoveBackWithTransData.Address, data);
            if (!GetModular().VerifyModuleActivityStatus(mechanicalArmMoveBackWithTransData.StateAddress))
            {
                await GetModular().ModuleExecuteAsync(mechanicalArmMoveBackWithTransData.CommandAddress, (short)mechanicalArmMoveBackWithTransData.ArmFuncCode);
            }
            await GetModular().CheckDoneAsync(mechanicalArmMoveBackWithTransData.StateAddress, mechanicalArmMoveBackWithTransData.CommandAddress);
